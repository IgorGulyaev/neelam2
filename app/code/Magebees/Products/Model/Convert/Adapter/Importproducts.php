...
class Importproducts extends \Magento\Framework\Model\AbstractModel
{
	...
	public function runImport(array $data)
	{
		...
		{
			$model = $_objectManager->create('\Magebees\Products\Model\Importlog');
			try {
				if(isset($e['txt'])){
	...
	public function saveRow(array $product)
	{
		...
		$behavior = $this->_request->getParam('behavior');
		if($behavior=='delete'){
			try{
				$product_id = $this->_objectManager->getIdBySku($product['sku']);
				if ($product_id) {
				...
				array_push($this->_error,array('txt'=>$e->getMessage(),'product_sku'=>$product['sku']));
			}
			return ;
		}
		...
		if(isset($product['type']) && $product['type'] != ""){
			...
			array_push($this->_error,array('txt'=>'Type is not define','product_sku'=>$product['sku']));
			return;
		}
		
		foreach ( $product as $field => $value ) {
			
			if($field == "price_view"){continue;} //added by parag
			if($field == "shipment_type"){continue;} //added by parag
		...
		$ProductImageGallery = $this->ProductImageGallery($product);
		...
		$this->CreateProductWithrequiredField($prodtype,$ProductData,$ProductAttributeData,$ProductImageGallery,$ProductStockdata,$ProductSupperAttribute,$ProductCustomOption);
	}
	public function CreateProductWithrequiredField($prodtype,$ProductData,$ProductAttributeData,$ProductImageGallery,$ProductStockdata,$ProductSupperAttribute,$ProductCustomOption){
		...
		if($prodtype == "simple"){
			 $this->_error = $this->SimpleProduct->SimpleProductData($ProductData,$ProductAttributeData,$ProductImageGallery,$ProductStockdata,$ProductSupperAttribute,$ProductCustomOption);
		}elseif($prodtype == "configurable"){
			 $this->_error = $this->_error = $this->ConfigurableProduct->ConfigurableProductData($ProductData,$ProductAttributeData,$ProductImageGallery,$ProductStockdata,$ProductSupperAttribute,$ProductCustomOption);
		}elseif($prodtype == "grouped"){
			 $this->_error = $this->GroupedProduct->GroupedProductData($ProductData,$ProductAttributeData,$ProductImageGallery,$ProductStockdata,$ProductSupperAttribute);
		}elseif($prodtype == "virtual"){
			 $this->_error = $this->VirtualProduct->VirtualProductData($ProductData,$ProductAttributeData,$ProductImageGallery,$ProductStockdata,$ProductSupperAttribute,$ProductCustomOption);
		}elseif($prodtype == "bundle"){
			 $this->_error = $this->BundleProduct->BundleProductData($ProductData,$ProductAttributeData,$ProductImageGallery,$ProductStockdata,$ProductSupperAttribute,$ProductCustomOption);
		}elseif($prodtype == "downloadable"){
			 $this->_error = $this->DownloadableProduct->DownloadableProductData($ProductData,$ProductAttributeData,$ProductImageGallery,$ProductStockdata,$ProductSupperAttribute,$ProductCustomOption);
		}else{
		}
	}
	...
	public function ProductData($product){
		...
		if(isset($product['categories'])) { $defaultProductData['categories'] = $this->addCategories($product['categories'], $product['store']); }
	...
	public function ProductSupperAttribute($product){
		$defaultSupperAttributeData = array(
			...
			'cws_tier_price' 			=> (isset($product['cws_tier_price'])) ? $this->TierPricedata($product['cws_tier_price'], $product['sku']) : '',
	...
	public function ProductImageGallery($product)
	{
		/* This code is Added for Manage without slash name image names */
		if(isset($product["image"]) != "" && substr_count($product["image"],"/") == 0){
			$product["image"] = "/".$product["image"];
		}
		if(isset($product["small_image"]) != "" && substr_count($product["small_image"],"/") == 0){
			$product["small_image"] = "/".$product["small_image"];
		}
		if(isset($product["thumbnail"]) != "" && substr_count($product["thumbnail"],"/") == 0){
			$product["thumbnail"] = "/".$product["thumbnail"];
		}
		if(isset($product["swatch_image"]) != "" && substr_count($product["swatch_image"],"/") == 0){
	...
	public function TierPricedata($TPData, $product_sku){
		
		//Remove Already Exits Tier Prices
		$pId = $this->_objectManager->getIdBySku($product_sku);
		$connectionRead = $this->getConnection('core_write');
		$_eav_attribute_set = $this->_resource->getTableName('catalog_product_entity_tier_price');
